...
RationaleArg = Union{Integer, Tuple{Integer, Integer}}
mutable struct Rationale
	op::Function
	args::Vector{RationaleArg}
end

Rationale(op::Function, args::RationaleArg...) = Rationale(op, args)
...
ProofStep(ϕ::Formula, rationale::Rationale) = ProofStep(false, ϕ, rationale, 0)
ProofStep(indent::Bool, ϕ::Formula, rationale::Rationale) = ProofStep(indent, ϕ, rationale, 0)
ProofStep(ϕ::Formula, rationale::Rationale, outdent::Int) = ProofStep(false, ϕ, rationale, outdent)
...
function andi(Γ, i, j)
	if Γ ≡ i && Γ ≡ j
		ProofStep(Γ(i) ∧ Γ(j), Rationale(andi, i, j))
	end
end
...
function ande1(Γ, i)
	if Γ ≡ i
		ϕ = Γ(i)
		if typeof(ϕ) == And
			ProofStep(ϕ.ϕ₁, Rationale(ande1, i))
		end
	end
end
...
function ande2(Γ, i)
	if Γ ≡ i
		ϕ = Γ(i)
		if typeof(ϕ) == And
			ProofStep(ϕ.ϕ₂, Rationale(ande2, i))
		end
	end
end
...
function ori1(Γ, i, ψ=nothing)
	if ψ ≠ nothing && Γ ≡ i
		ψ = parse_formula(ψ)
		ProofStep(Γ(i) ∨ ψ, Rationale(ori1, i))
	end
end
...
function ori2(Γ, i, ψ=nothing)
	if ψ ≠ nothing && Γ ≡ i
		ψ = parse_formula(ψ)
		ProofStep(ψ ∨ Γ(i), Rationale(ori2, i))
	end
end
...
function ore(Γ, i, J, K)
	...
	if Γ ≡ i && Γ ≡ J && Γ ≡ K
		...
		if typeof(Γ(i)) == Or && χ₁ == χ₂
			ProofStep(χ₁, Rationale(ore, i, J, K))
		end
	end
end
...
function impi(Γ, I)
	i₁, i₂ = I
	if Γ ≡ I
		ProofStep(Γ(i₁) → Γ(i₂), Rationale(impi, I))
	end
end
...
function impe(Γ, i, j)
	if Γ ≡ i && Γ ≡ j
		...
		if typeof(ϕ) == Imp && Γ(j) == ϕ.ϕ₁
			ProofStep(ϕ.ϕ₂, Rationale(impe, i, j))
		end
	end
end
...
function negi(Γ, I)
	i₁, i₂ = I
	if Γ ≡ I && Γ(i₂) == False()
		ProofStep(¬Γ(i₁), Rationale(negi, I))
	end
end
...
function nege(Γ, i, j)
	if Γ ≡ i && Γ ≡ j
		...
		if typeof(ϕ₂) == Neg && ϕ₁ == ϕ₂.ϕ
			ProofStep(False(), Rationale(nege, i, j))
		end
	end
end
...
function bote(Γ, i, ψ=nothing)
	if ψ ≠ nothing && Γ ≡ i && Γ(i) == False()
		ψ = parse_formula(ψ)
		ProofStep(ψ, Rationale(bote, i))
	end
end

function negnegi(Γ, i)
	if Γ ≡ i
		ProofStep(¬¬Γ(i), Rationale(negnegi, i))
	end
end
...
function negnege(Γ, i)
	if Γ ≡ i
		...
		if typeof(ϕ) == Neg
			ϕ = ϕ.ϕ
			if typeof(ϕ) == Neg
				ProofStep(ϕ.ϕ, Rationale(negnege, i))
			end
		end
	end
end
...
function MT(Γ, i, j)
	if Γ ≡ i && Γ ≡ j
		...
		if typeof(ϕ₁) == Imp && typeof(ϕ₂) == Neg && ϕ₁.ϕ₂ == ϕ₂.ϕ
			ProofStep(¬ϕ₁.ϕ₁, Rationale(MT, i, j))
		end
	end
end
...
function assume(Γ, ψ=nothing)
	if ψ ≠ nothing
		ψ = parse_formula(ψ)
		ProofStep(true, ψ, Rationale(assume))
	end
end
...
function copy(Γ, i)
	if Γ ≡ i
		ProofStep(Γ(i), Rationale(copy, i))
	end
end
